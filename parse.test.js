...
import { parse as parseTwo } from './parse-poc.js'

const allMethods = ['fastParse', 'slowParse']
const quoteMethods = ['slowParse']

const start = () => {
  return process.hrtime.bigint()
}
const stop = (start) => {
  return Number.parseInt((process.hrtime.bigint() - start).toString()) / 1000000
}

test('Speed: Should parse single row with { }', async (t) => {
  const options = {}
  const enqueue = sinon.spy()
  const chunk = 'a,b,c\n1,2,3\n4,5,6\n'
  const parser = parse(options)
  const { regexpExecParse } = parseTwo(options)
  let timer
  ...
  for (const method of allMethods) {
    ...
    console.log(method, stop(timer), 'ms')
  }

  timer = start()
  regexpExecParse(chunk, { enqueue })
  console.log(stop(timer), 'ms')

  equal(enqueue.callCount, 20)
})

test('Speed: Should parse single row with { } w/ quotes', async (t) => {
  const options = {}
  const enqueue = sinon.spy()
  const chunk = 'a,b,c\n"1","2","3"\n"4","5","6"\n'
  const parser = parse(options)
...
for (const method of allMethods) {
  ...
  test(`${method}: Should parse multiple rows with { }`, async (t) => {
    ...
    const parser = parse(options)
    parser[method](chunk, { enqueue })
    parser[method](parser.previousChunk(), { enqueue })
...
// *** Option: fastMode *** //
/* test(`${method}: Should parse with { fastMode: true }`, async (t) => {
  const options = { fastMode: true }
  const enqueue = sinon.spy()
  const chunk = 'a,b,c\n1,2,3\n'
  equal(canUseFastMode(chunk, options), true)
})

test(`${method}: Should parse with { fastMode: true } adn quotes`, async (t) => {
  const options = { fastMode: true }
  const enqueue = sinon.spy()
  const chunk = 'a,b,c\n1,"2",3\n'
  equal(canUseFastMode(chunk, options), false)
})

test(`${method}: Should parse with { fastMode: false }`, async (t) => {
...
for (const method of quoteMethods) {
  test(`${method}: Should parse with { errorOnFieldMalformed: false }`, async (t) => {
  ...
  test(`${method}: Should parse with { errorOnFieldMalformed: true }`, async (t) => {
